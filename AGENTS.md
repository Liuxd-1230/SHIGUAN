...
未建立：真实 token 表下发的中文化收尾（M3.2 之后）、记忆时间线并入契约时间线的去重合并（M5 TimelineBuilder）、传记管线 LLM（Phase 3）、家族树/地点/导出（Phase 4）。
下一轮见 `docs/roadmap.md` 的 Phase 2B M5+ 与 Phase 3。
