...
_FIELD_TABLE: dict[tuple[str, str], tuple[bool, bool, str | None, str]] = {
    ("capital", "landed_titles"): (True, True, None, "3C.7 P1：reader 读取 capital_title_id（数字引用反查 key），进入 TitleStructure"),
    ("claims", "landed_titles"): (True, False, "SEMANTIC_UNUSED", "3C.7 P1：reader 读取 claimant_ids（与人物实际持有分开），进入 TitleStructure"),
    ("de_jure_liege", "landed_titles"): (True, True, None, "3C.7 P1：reader 读取 de_jure_liege_id，进入 TitleStructure"),
    ("history_government", "landed_titles"): (True, False, "SEMANTIC_UNUSED", "3C.7 P1：reader 读取（标量/块形态），保留结构化契约，不生成复杂文案"),
    ...
    ("de_jure_vassals", "landed_titles"): (True, True, None, "3C.7 P1：reader 读取 de_jure_vassal_ids，进入 TitleStructure"),
    ...
    ("was_player", "character"): (True, True, None, "3C.7 P1：reader 读取 playable_data.was_player → PlayerHistoryMarker（isCurrentPlayer 由 meta.player_id 匹配）"),
    ("landed_data", "character"): (True, True, None, "reader 用它判定 ruler 并读取 domain"),
    ...
    ("capital", "character"): (False, False, "READER_DROPPED", "存档人物块无 capital= 直接字段（政权中心在 title 顶层 capital，已由 reader 读取）"),
    ("government", "character"): (False, False, "READER_DROPPED", "存档人物块有 government 字段（landed_data），reader 未读取"),
    ("domain", "character"): (True, True, None, "3C.7 P1：reader 读取 landed_data.domain → CharacterDomain，并与 title holder 反查互相校验（不一致降 confidence，不静默选边）"),
